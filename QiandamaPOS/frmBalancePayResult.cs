...
namespace QiandamaPOS
{
    public partial class frmBalancePayResult : Form
    {
        ...
        private void AuthCodeTrade()
        {
            ...
            {
                string ErrorMsg = "";
                BalanceTrade balancetrade = httputil.BalanceTrade(CurrentOrderID, CurrentAuthCode, ref ErrorMsg);

                if (ErrorMsg != "" || balancetrade == null) //商品不存在或异常
                {
                ...
                {
                    if (balancetrade.status == "SUCCESS")
                    {
                        if (DataReceiveHandle != null)
                            this.DataReceiveHandle.BeginInvoke(1, balancetrade.outtradeno, null, null);
                        isrun = false;
                        this.Close();
                    }
                    else
                    {
                        ...
                        CurrentPayID = balancetrade.id;
                        SyncTrade(CurrentOrderID, balancetrade.id);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void SyncTrade(string orderid, string payid)
        {
            try
            {
                ...
                string errormsg = "";
                BalanceTrade sync = httputil.QueryTrade(orderid, payid, ref errormsg);
                ...
                else if (sync.status == "SUCCESS")
                {
                    ...
                    pnlWaiting.Visible = false;
                    if (DataReceiveHandle != null)
                        this.DataReceiveHandle.BeginInvoke(1, sync.outtradeno, null, null);
                    isrun = false;
                    this.Close();
                }
                if (isrun)
        ...
        private void timerAuthCodeTrade_Tick(object sender, EventArgs e)
        {
            ...
            if ((DateTime.Now - StartTime).TotalMinutes > 1 || (DateTime.Now - StartTime).TotalSeconds > 600)
            {
                MainModel.ShowLog("交易超时失败！", false);
                isrun = false;
            }
