...
namespace QiandamaPOS.Common
{
    ...
    public class AutoSizeFormUtil
    {
        ...
        private void AutoScaleControl(Control mForm, float wScale, float hScale)
        {
            ...
            if (mForm.Name.Length > 0 && mForm.Name.Substring(0, 3) == "dgv")
            {
                name = "";
            }
        ...
        float wScale = 1;
        float hScale = 1;
        public void AutoScaleControlTest(Control mForm,float width,float height,bool isParent)
        {
            ...
            if (isParent)
            {
                wScale = (float)width / mForm.Width;
                hScale = (float)height / mForm.Height;
                mForm.Size = new System.Drawing.Size((int)Math.Ceiling(mForm.Size.Width * wScale), (int)Math.Ceiling(mForm.Size.Height * hScale));
            }
            ...
            foreach (Control c in mForm.Controls)
            {
                ...
                if (c.Controls.Count > 0)
                {
                    AutoScaleControlTest(c,0,0,false);
                }
            }
        }
