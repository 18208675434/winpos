        ...
        private void LoadPnlCategory(SortedDictionary<string, string> sort)
        {
            try
            {
                foreach (KeyValuePair<string, string> kv in sort)
                {
                    ...
                    btntemp.Font = new System.Drawing.Font("微软雅黑", 10F * Math.Min(MainModel.hScale, MainModel.wScale), System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(134)));
                    btntemp.Size = new System.Drawing.Size(pnlCategory.Width, Convert.ToInt32(55 * Math.Min(MainModel.hScale, MainModel.wScale)));
        ...
        private void btnCategory_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (Control con in pnlCategory.Controls)
                {
                    ...
                    con.Font = new System.Drawing.Font("微软雅黑", 10F * Math.Min(MainModel.hScale, MainModel.wScale), FontStyle.Regular);
                }

                btn.BackColor = Color.White;
                btn.Font = new System.Drawing.Font("微软雅黑", 11F * Math.Min(MainModel.hScale, MainModel.wScale), FontStyle.Bold);
            ...
            {
                LogManager.WriteLog("选择面板商品分类异常"+ex.Message);
            }
            finally
            {
                IsEnable = true;
        ...
        private void lblExit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (CurrentCart!=null && CurrentCart.products!=null && CurrentCart.products.Count>0)
                {
                    ...
                    if (!WinSaasPOS_Scale.HelperUI.ConfirmHelper.Confirm("确认返回", "返回后将清空已选商品，确认返回？"))
                    {
                        return;
                    }
        ...
        private void dgvGoodPic_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (selectpro.weightflag)
                {
                    ...
                    string numbervalue = NumberHelper.ShowFormNumber(selectpro.skuname, NumberType.ProWeight);
                    if (!string.IsNullOrEmpty(numbervalue))
                    {
                        selectpro.specnum = Convert.ToDecimal(numbervalue) / 1000;
                        selectpro.price.specnum = Convert.ToDecimal(numbervalue) / 1000;
                        selectpro.num = 1;
                ...
                foreach (Product tpro in CurrentCart.products)
                {
                    if (tpro.barcode == pro.barcode)
                    {
                        if (tpro.num == 1)
                        {
                            ...
                            if (WinSaasPOS_Scale.HelperUI.ConfirmHelper.Confirm("是否确认删除商品？"))
                            {
