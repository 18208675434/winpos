...
//                cartBean.setListScope(listScope); //适用的商品行号
        ...
        private bool canApplySpecificPromo(DBPROMOTION_CACHE_BEANMODEL promotion, Product orderItem, String shopId)
        {
            ...
            {
                //部分分类不参与夜间促销
                JSON_BEANMODEL jsonmodel = jsonbll.GetModel(ConditionType.PromotionsInvalidBean);
                if (jsonmodel != null && !string.IsNullOrEmpty(jsonmodel.JSON))
                {
                    PromotionsInvalidBean promotionsInvalidBean = JsonConvert.DeserializeObject<PromotionsInvalidBean>(jsonmodel.JSON);
                    if (promotionsInvalidBean != null)
                    {
                        if ((promotionsInvalidBean.invalidcategories != null && promotionsInvalidBean.invalidcategories.Contains(orderItem.firstcategoryid)) || (promotionsInvalidBean.invalidskucodes != null && promotionsInvalidBean.invalidskucodes.Contains(orderItem.skucode)))
                        {
                            return false;
                        }
                    }
