...
namespace ZhuiZhi_Integral_Scale_UncleFruit
{
    public partial class frmTest : Form
    {
        ...
        {
            KB.Size = new Size(Convert.ToInt32(txtWidth.Text),Convert.ToInt32(txtheight.Text));

            //KB.Press += MiniKeyboardHandler;
        }

        string keyInput = "";
        private void MiniKeyboardHandler(object sender, ZhuiZhi_Integral_Scale_UncleFruit.MyControl.KeyBoard.KeyboardArgs e)
        {
            TextBox focusing = textBox1;
            keyInput = e.KeyCode;

            int startDel = 0;

            //退格
            if (keyInput == KB.KeyDelete)
            {
            ...
            //按确定，焦点转移
            else if (keyInput == KB.KeyEnter)
            {
                //TOOD querendong
            }
            else if (keyInput == KB.KeyClear)
            {
                focusing.Text = "";
            }
            else if (keyInput == KB.KeyHide)
            {
                KB.Hide();
            }
