...
{
    public partial class FormMemberOrderQuery : Form
    {
        ...
        #region  页面加载与退出
        public FormMemberOrderQuery(string memberid)
        {
            InitializeComponent();
            // Application.EnableVisualStyles();
            CurrentCustomerID = memberid;
        }
        ...
        private void LoadDgvOrder()
        {
            ...
            queryorderpara.customerid = MainModel.CurrentMember.memberheaderresponsevo.memberid;

               // queryorderpara.customerid=
            string ErrorMsg = "";
            ...
            try
            {
                ...
                foreach (Order order in lstOrder)
                {
                   ...
                   if (order.orderstatusvalue == 5)
                    {
                        dgvOrderOnLine.Rows.Add(MainModel.GetDateTimeByStamp(order.orderat.ToString()).ToString("yyyy-MM-dd HH:mm:ss"), order.orderid, order.customerphone, order.title, totalpay, order.orderstatus, bmpDetail);
                    }
                    else
                    {
        ...
        private void LoadOtherOnline()
        {
            try
            {
                ...
                foreach (Order order in lstOrder)
                {
                        ...
                        dgvOnLine.Rows.Add(MainModel.GetDateTimeByStamp(order.orderat.ToString()).ToString("yyyy-MM-dd HH:mm:ss"), order.orderid, order.customerphone, order.title, order.ordersubtype, order.orderstatus,order.receiverphone );
                }
