...
namespace ZhuiZhi_Integral_Scale_UncleFruit.BaseUI
{
    public class MainHelper
    {
        ...
        private static ImplOfflineAdjsutPrice impadjust = new ImplOfflineAdjsutPrice();
        ...
        public static void UpdatePaymentType(Cart cart)
        {
            try
            {
                ...
                if (HalfOffLineUtil.paymenttypes == null)
                {
                    Thread threadmqtt = new Thread(HalfOffLineUtil.GetAvailablePaymentTypes);
        ...
        public static void SingleCalculate(List<Product> lstpro)
        {
            ...
            //初始化商品数据
            foreach (Product singlepro in lstpro)
            {
        ...
        /// </summary>
        /// <param name="pro"></param>
        public static void AdjustCalculate(Cart cart)
        {

            //初始化商品数据
            foreach (Product singlepro in cart.products)
            {
                impadjust.adjustPrice(singlepro);               
            }
        ...
        {
            //2020-06-30  计算之前清空，防止减少商品优惠券不适合
            cart.availablecoupons = null;
