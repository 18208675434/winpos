            ...
            if (products != null && products.Count > 0) {
                for (int i = 0; i < products.Count; i++) {
                    ...
                    if (tempProduct != null) {
                        ...
                        if (PromotionCache.getCategoryIds(tempProduct) == null)
                       {
                           continue;
                       }

                        if (CollectionUtils.isNotEmpty(realmDetail.catalogsToExclude) && !GlobalUtil.IsArrayIntersection(realmDetail.catalogsToExclude,PromotionCache.getCategoryIds(tempProduct)) )
                        {
                            continue;
                        }
