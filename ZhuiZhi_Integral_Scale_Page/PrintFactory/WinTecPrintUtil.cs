        ...
        public static bool PrintThirdOrder(PrinterPickOrderInfo printdetail, ref string errormsg)
        {
            lock (lockthirdprinting)
            {
                try
                {
                    ...
                    PrintText(PrintHelper.MergeStr(MainModel.CurrentShopInfo.tenantname, "", HeadCharCountOfLine, PageSize) + "\n", 32);
                    PrintText(PrintHelper.MergeStr(MainModel.CurrentShopInfo.shopname, "", HeadCharCountOfLine, PageSize) + "\n", 32);
                    PrintText(printdetail.serialcode + "\n", 40);

                    PrintText("订单号：" + printdetail.orderid + "\n", 25);
                    PrintText("下单时间：" + printdetail.date + "\n", 25);
                    PrintText("顾客姓名：" + printdetail.username + "\n", 25);
                    PrintText("顾客电话：" + printdetail.tel + "\n", 25);
                    ...
                    {
                        PrintText(printdetail.remark + "\n", 32);
                    }
                    PrintText("期望送达时间：" + "\n", 32);
                    PrintText( printdetail.expecttimedesc + "\n", 32);
                    ...
                    PrintText(PrintHelper.MergeStr("实付金额：", printdetail.totalpayment.ToString("f2"), BodyCharCountOfLine, PageSize) + "\n", 25);

                    if (!string.IsNullOrEmpty(printdetail.pickcode))
                    {
                        PrintText(PrintHelper.getStrLine() + "\n", 25);
                        PrintText("请扫描下方二维码取货配送" + "\n", 25);
                        PrintText("  " + "\n", 25);
                        BeginPrint(0);
