...
namespace QDAMAPOS
{
    public partial class frmMainOffLine : Form
    {
      ...
      //  HttpUtil httputil = new HttpUtil();
        ...
        private DBPRODUCT_BEANMODEL GetLocalPro(string barcode)
        {
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            dbpro.NUM  = 1;
                            dbpro.SPECNUM = (decimal)pronum / 1000;
                        }
                ...
                try
                {
                    ...
                    if (CurrentCart.products != null && !pro.weightflag)
                    {
                        ...
                        foreach (Product exitspro in CurrentCart.products)
                        {
                            ...
                            {
                                exitspro.num++;
                                exitspro.price.total = Math.Round(exitspro.num * exitspro.price.saleprice, 2);
                                exitspro.price.origintotal = Math.Round(exitspro.num * exitspro.price.originprice, 2);
                                exitspro.PaySubAmt = Math.Round(exitspro.num * exitspro.price.saleprice, 2);
                                newpro = false;
                                break;
                            }
                        }
                        ...
                        if (newpro)
                        {
                            pro.price.total = Math.Round(pro.price.saleprice, 2);
                            pro.price.origintotal = Math.Round(pro.price.originprice, 2);
                            pro.PaySubAmt = Math.Round(pro.price.saleprice, 2);
                            CurrentCart.products.Add(pro);
                        }
                    }
                    else
                    {
                        pro.price.total = Math.Round(pro.price.saleprice * pro.price.specnum, 2);
                        pro.price.origintotal = Math.Round(pro.price.originprice * pro.price.specnum, 2);
                        pro.PaySubAmt = Math.Round(pro.price.saleprice * pro.price.specnum, 2);
                        CurrentCart.products.Add(pro);
                ...
                try
                {
                    ...
                    if (cart != null && cart.products != null && cart.products.Count > 0)
                    {
                        foreach (Product singlepro in cart.products)
                        {
                            ...
                            {
                                singlepro.price.total = Math.Round(singlepro.num * singlepro.price.saleprice, 2);
                                singlepro.price.origintotal = Math.Round(singlepro.num * singlepro.price.originprice, 2);
                                singlepro.PaySubAmt = Math.Round(singlepro.num * singlepro.price.saleprice, 2);

                            }
                            else
                            {
                                singlepro.price.total = Math.Round(singlepro.price.saleprice * singlepro.price.specnum, 2);
                                singlepro.price.origintotal = Math.Round(singlepro.price.originprice * singlepro.price.specnum, 2);
                                singlepro.PaySubAmt = Math.Round(singlepro.price.saleprice * singlepro.price.specnum, 2);
        ...
        private void UploadOrderDetail()
        {
            lock (thislockOrderDetail)
            {
                try
                {
                    ...
                    CurrentCart.totalpayment = Math.Round(carttotal, 2);
                    ...
                    if (cartpromoamt > 0)
                    {
                        ...
                        CurrentCart.promoamt = Math.Round(cartpromoamt, 2);
                    }
            ...
            try
            {
                ...
                if (CurrentCart != null && CurrentCart.products != null && CurrentCart.products.Count > 0)
                {
                    ...
                    if (type == 0)
                    {
                        this.Invoke(new InvokeHandler(delegate()
                        {
                            frmOnLinePayResultBack frmonlinepayresultback = new frmOnLinePayResultBack(orderid, frmCashPayOffLineBack.CurrentCart);
                ...
                if (e.ColumnIndex == 3 && pro.goodstagid == 0)
                {
                    for (int i = 0; i < CurrentCart.products.Count; i++)
                    {
                        if (CurrentCart.products[i].skucode == pro.skucode)
                        {
                            ...
                            CurrentCart.products[i].price.total = Math.Round(CurrentCart.products[i].num * CurrentCart.products[i].price.saleprice,2);
                            CurrentCart.products[i].price.origintotal = Math.Round(CurrentCart.products[i].num * CurrentCart.products[i].price.originprice, 2);
                           // CurrentCart.products[i].PaySubAmt = Math.Round(CurrentCart.products[i].num * CurrentCart.products[i].price.saleprice, 2);
        ...
        private void btnMianban_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    //防止序列化死循环
                    foreach (Product proclear in frmpanel.SelectProducts)
                    {
                    ...
                    {
                        //防止序列化死循环
                        foreach (Product pro in frmpanel.SelectProducts)
                        {
                            if (pro.weightflag)
                            {
                                CurrentCart.products.Add(pro);
