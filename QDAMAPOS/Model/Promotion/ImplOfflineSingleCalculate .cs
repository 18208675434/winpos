  ...
  public class ImplOfflineSingleCalculate {
      ...
      public void calculate(Product productBean)
      {
          try
          {
              ...
              //if (productBean.skucode == "20014490")
              //{
              //    string test ="";
              //    test+="";
              //}
              ...
              if (currentPriceTriplet != null)
              {
                  if (productBean.price != null && currentPriceTriplet.getPromoTriplet() != null && !string.IsNullOrEmpty(currentPriceTriplet.getPromoTriplet().CODE) && !string.IsNullOrEmpty(currentPriceTriplet.getPromoTriplet().COSTCENTERINFO))
                  {
                      ...
                      if (!productBean.weightflag)
                      {
                          ...
                          productBean.price.total = Math.Round(productBean.num * productBean.price.saleprice, 2);
                          productBean.price.origintotal = Math.Round(productBean.num * productBean.price.originprice, 2);
                          productBean.PaySubAmt = Math.Round(productBean.num * productBean.price.saleprice, 2);

                      }
                      else
                      {
                          productBean.price.total = Math.Round(productBean.price.saleprice * productBean.price.specnum, 2);
                          productBean.price.origintotal = Math.Round(productBean.price.originprice * productBean.price.specnum, 2);
                          productBean.PaySubAmt = Math.Round(productBean.price.saleprice * productBean.price.specnum, 2);
    ...
    private TripletBean getNewPriceAndKind(DBPROMOTION_CACHE_BEANMODEL dbPromotionCacheBean, Product productBean) {
        ...
        if (MoneyUtils.isFirstBiggerThanOrEqualToSecond(productBean.price.originprice, newPrice)) {
            ...
            if (!"qdm.fixed.price".Equals(dbPromotionCacheBean.PROMOCONDITIONCONTEXT) && EnumPromotionType.PROMOTION_PRICE_DISCOUNT_ACTION.Equals(dbPromotionCacheBean.PROMOACTION)) {//打折
            ...
            int pricetagid = -1;
            String pricetag = "";
            if (EnumPromotionType.PROMOTION_PRICE_DISCOUNT_ACTION.Equals(dbPromotionCacheBean.PROMOACTION) && !"qdm.fixed.price".Equals(dbPromotionCacheBean.PROMOCONDITIONCONTEXT)) {
    ...
    private Decimal getCalculationPrice(DBPROMOTION_CACHE_BEANMODEL dbPromotionCacheBean, Product productBean) {
        if (!"qdm.fixed.price".Equals (dbPromotionCacheBean.PROMOCONDITIONCONTEXT) &&
                EnumPromotionType.PROMOTION_PRICE_DISCOUNT_ACTION.Equals(dbPromotionCacheBean.PROMOACTION)) {//打折
            Decimal discount =Math.Round( MoneyUtils.newMoney(dbPromotionCacheBean.PROMOACTIONCONTEXT),2);
